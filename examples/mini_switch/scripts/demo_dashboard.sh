#!/bin/bash
# Demo script with real-time dashboard

set -e

echo "=== Mini Switch Dashboard Demo ==="
...
cleanup() {
    ...
    pkill -f "mini_switch_dashboard" 2>/dev/null || true
    sleep 1
...
echo "Starting mini-switch with dashboard..."
echo "You will see a real-time visualization dashboard."
echo ""

# Check if binary exists
if [ ! -f "./bin/mini_switch_dashboard" ]; then
    echo "Error: mini_switch_dashboard binary not found. Building first..."
    make dashboard
fi

# Start mini-switch in background with dashboard mode
echo "Executing: sudo ./bin/mini_switch_dashboard veth_sw1 veth_sw2 --dashboard"
sudo ./bin/mini_switch_dashboard veth_sw1 veth_sw2 --dashboard &
SW_PID=$!
...
echo "• Watch real-time packet flow"
