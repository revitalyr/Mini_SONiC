...
// Windows Visual Demo Configuration
...
// Windows Visual TUI with pure ASCII
class WindowsVisual {
private:
    int switch_count;
...
public:
    WindowsVisual(int switches) : switch_count(switches) {
...
#ifdef _WIN32
        // Set console code page to Windows-1252 for better ASCII support
    ...
    void show_header(const WindowsStats& stats) {
        std::cout << "\n";
        std::cout << "+======================================================================+\n";
        std::cout << "|              Mini SONiC - WINDOWS VISUAL NETWORK DEMO            |\n";
    ...
    void show_network_topology(const std::vector<std::unique_ptr<std::atomic<uint64_t>>>& switch_packets,
        ...
        for (int i = 0; i < switch_count; ++i) {
            ...
            std::cout << "  | " << std::setw(18) << std::left << switch_names[i] << " | ";
            if (i < switch_count - 1) {
                std::cout << "     |       ";
            }
            std::cout << "\n";
    ...
    void show_data_flows() {
        ...
        // Show animated data flows with pure ASCII
    ...
    void show_statistics(const WindowsStats& stats) {
        std::cout << "+----------------------------------------------------------------------+\n";
        std::cout << "|                        STATISTICS                                 |\n";
        std::cout << "+----------------------------------------------------------------------+\n\n";
        
        std::cout << "  [DATA] Packets Processed:    " << std::setw(6) << stats.packets_processed.load() << "\n";
        std::cout << "  [X]    Packets Dropped:      " << std::setw(6) << stats.packets_dropped.load() << "\n";
        std::cout << "  [SPEED] Processing Rate:    " << std::fixed << std::setprecision(1) 
        ...
        std::cout << "  [MAC]  MAC Addresses:       " << std::setw(4) << stats.mac_learning_events.load() << "\n\n";
    }
    
    void show_progress(const WindowsStats& stats, int elapsed_seconds, int total_seconds) {
    ...
    void show_final_stats(const WindowsStats& stats, 
        ...
        std::cout << "+----------------------------------------------------------------------+\n";
        std::cout << "|                    WINDOWS VISUAL ANIMATION COMPLETED           |\n";
        std::cout << "+----------------------------------------------------------------------+\n\n";
        
        std::cout << "FINAL RESULTS:\n";
        std::cout << "  [DATA] Packets Processed:      " << std::setw(6) << stats.packets_processed.load() << "\n";
        std::cout << "  [X]    Packets Dropped:        " << std::setw(6) << stats.packets_dropped.load() << "\n";
        std::cout << "  [SPEED] Average Throughput:     " << std::fixed << std::setprecision(1) 
                  << std::setw(6) << stats.get_packets_per_second() << " pps\n";
        std::cout << "  [%]    Packet Loss Rate:        " << std::fixed << std::setprecision(2) 
                  << std::setw(5) << stats.get_packet_loss_rate() << "%\n";
        std::cout << "  [FRAME] Total Frames:           " << std::setw(6) << stats.frame_number.load() << "\n\n";
        ...
        std::cout << "*** Mini SONiC: Windows Visual Network Animation Complete! ***\n";
    }
...
// Windows Visual Demo Controller
class WindowsVisualDemo {
private:
    WindowsConfig config;
    WindowsStats stats;
    WindowsVisual visualizer;
...
public:
    WindowsVisualDemo(const WindowsConfig& cfg) 
        ...
        std::cout << "Starting WINDOWS VISUAL animation with " << config.num_switches << " switches...\n";
        std::cout << "Rate: " << config.packets_per_second << " packets/second (Windows compatible)\n";
        std::cout << "Duration: " << config.demo_duration_seconds << " seconds\n";
        std::cout << "Refresh: " << config.refresh_interval_ms << "ms (visible animation)\n";
        std::cout << "*** WINDOWS VISUAL MODE - Pure ASCII, Windows Compatible ***\n\n";
    }
    ...
    void start() {
        ...
        // Start demo thread
        demo_thread = std::make_unique<std::thread>(&WindowsVisualDemo::run_demo, this);
    }
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--switches" && i + 1 < argc) {
            ...
            std::cout << "Mini SONiC Windows Visual Network Animation Demo\n";
            ...
            std::cout << "\nFeatures:\n";
            std::cout << "  - Windows Visual TUI mode\n";
            std::cout << "  - Real network topology visualization\n";
            std::cout << "  - Animated data flows and switches\n";
            std::cout << "  - Frame-by-frame display\n";
            std::cout << "  - Pure ASCII characters (Windows compatible)\n";
            std::cout << "  - No Mini SONiC dependencies\n";
            std::cout << "  - Windows console optimization\n";
            return 0;
        }
    }
    
    try {
        // Create and run Windows visual demo
        WindowsVisualDemo demo(config);
        ...
        std::cerr << "Windows Visual Demo failed with error: " << e.what() << std::endl;
        return 1;
    }
}
