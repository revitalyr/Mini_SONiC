...
// Pure TUI Demo Configuration
struct PureTUIConfig {
    int num_switches = 3;
...
public:
    PureTUI(int switches) : switch_count(switches) {
    ...
    void show_header(const PureStats& stats) {
        std::cout << "\n";
        std::cout << "================================================================\n";
        std::cout << "           Mini SONiC - PURE TUI ANIMATION           \n";
    ...
    void show_progress(const PureStats& stats, int elapsed_seconds, int total_seconds) {
    ...
    void show_statistics(const PureStats& stats) {
        std::cout << "STATISTICS:\n";
    ...
    void show_topology(const std::vector<std::unique_ptr<std::atomic<uint64_t>>>& switch_packets,
        ...
        std::cout << "NETWORK TOPOLOGY - Pure TUI:\n\n";
    ...
    void show_final_stats(const PureStats& stats, 
                         const std::vector<std::unique_ptr<std::atomic<uint64_t>>>& switch_packets,
                         const std::vector<std::unique_ptr<std::atomic<uint64_t>>>& mac_tables) {
        clear_screen();
        show_header(stats);
        
        std::cout << "PURE TUI ANIMATION COMPLETED!\n\n";
        ...
        std::cout << "Mini SONiC: Pure TUI Animation Complete!\n";
    }
...
// Pure TUI Demo Controller
class PureTUIDemo {
private:
    PureTUIConfig config;
    PureStats stats;
    PureTUI visualizer;
...
public:
    PureTUIDemo(const PureTUIConfig& cfg) 
        ...
        std::cout << "Starting PURE TUI animation with " << config.num_switches << " switches...\n";
        ...
        std::cout << "*** PURE TUI MODE - No Mini SONiC dependencies ***\n\n";
    }
    ...
    void start() {
        ...
        // Start demo thread
        demo_thread = std::make_unique<std::thread>(&PureTUIDemo::run_demo, this);
    }
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--switches" && i + 1 < argc) {
            ...
            std::cout << "Mini SONiC Pure TUI Animation Demo\n";
            ...
            std::cout << "\nFeatures:\n";
            std::cout << "  - Pure TUI (Text User Interface) mode\n";
    ...
    try {
        // Create and run pure TUI demo
        PureTUIDemo demo(config);
        ...
        std::cerr << "Pure TUI Demo failed with error: " << e.what() << std::endl;
        return 1;
    }
}
