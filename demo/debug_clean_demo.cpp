...
// Simple test without Mini SONiC components first
using namespace std::chrono_literals;

// Test configuration
struct TestConfig {
    int num_switches = 3;
    int packets_per_second = 2;  // Very slow
    int demo_duration_seconds = 45;  // Longer
    bool enable_animation = true;
    bool enable_colors = true;
    int refresh_interval_ms = 2000;  // Very slow refresh
};
...
public:
    TestVisualizer(bool enable_anim, bool enable_colors, int switches) 
    ...
    void show_header() {
        ...
        std::cout << "\n";
        std::cout << "================================================================\n";
        std::cout << "           Mini SONiC - TEST ANIMATION        \n";
        std::cout << "              Pure Animation Test        \n";
        std::cout << "================================================================\n\n";
        set_color(WHITE);
    }
    
    void show_progress(const TestStats& stats, int elapsed_seconds, int total_seconds) {
        if (!enabled) return;
    ...
    void show_topology(const std::vector<std::unique_ptr<std::atomic<uint64_t>>>& switch_packets,
        ...
        set_color(YELLOW);
        std::cout << "NETWORK TOPOLOGY - Test Animation:\n\n";
    ...
    void show_final_stats(const TestStats& stats, 
        ...
        set_color(GREEN);
        std::cout << "*** TEST ANIMATION COMPLETED! ***\n\n";
        ...
        set_color(GREEN);
        std::cout << ">>> Test completed - animation working! <<<\n";
        set_color(WHITE);
    }
    ...
    void log_event(const std::string& event) {
        // Show some events for debugging
        if (enabled) {
            set_color(CYAN);
            std::cout << ">>> " << event << " <<<\n";
            set_color(WHITE);
...
public:
    TestDemo(const TestConfig& cfg) 
        ...
        std::cout << "Starting TEST animation with " << config.num_switches << " switches...\n";
        std::cout << "Rate: " << config.packets_per_second << " packets/second (VERY slow for testing)\n";
        std::cout << "Duration: " << config.demo_duration_seconds << " seconds\n";
        std::cout << "Refresh: " << config.refresh_interval_ms << "ms (VERY slow for visibility)\n";
        std::cout << "*** PURE ANIMATION TEST - No Mini SONiC components ***\n\n";
        
        visualizer.log_event("TEST DEMO STARTED");
    }
    ...
    void start() {
        ...
        // Start demo thread
        demo_thread = std::make_unique<std::thread>(&TestDemo::run_demo, this);
    }
    ...
    void stop() {
        ...
        visualizer.log_event("TEST DEMO COMPLETED");
    }
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--switches" && i + 1 < argc) {
            ...
            std::cout << "Mini SONiC Test Animation Demo\n";
            std::cout << "Usage: " << argv[0] << " [options]\n";
            std::cout << "Options:\n";
            std::cout << "  --switches <n>     Number of switches (default: 3)\n";
            std::cout << "  --rate <n>         Packets per second (default: 2)\n";
            std::cout << "  --duration <n>     Demo duration in seconds (default: 45)\n";
            std::cout << "  --refresh <n>      Refresh interval in ms (default: 2000)\n";
            ...
            std::cout << "  - No Mini SONiC components\n";
            return 0;
        }
    }
    
    try {
        // Create and run test demo
        TestDemo demo(config);
