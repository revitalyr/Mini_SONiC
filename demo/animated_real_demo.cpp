...
// Animated demo configuration
struct AnimatedDemoConfig {
    int num_switches = 3;
...
// Real-time visualizer with actual animation
...
public:
    AnimatedVisualizer(bool enable_anim, bool enable_colors, int switches) 
    ...
    void show_header() {
        ...
        std::cout << "\n";
        std::cout << "================================================================\n";
        std::cout << "           Mini SONiC - Real Network Processing Demo        \n";
        std::cout << "              Using Real Components - Live Animation         \n";
        std::cout << "================================================================\n\n";
        set_color(WHITE);
    }
    
    void show_progress(const PerformanceStats& stats, int elapsed_seconds, int total_seconds) {
        if (!enabled) return;
    ...
    void show_animated_switches(const std::vector<std::unique_ptr<std::atomic<uint64_t>>>& switch_packets,
                               const std::vector<std::unique_ptr<std::atomic<size_t>>>& mac_tables) {
        if (!enabled) return;
        
        set_color(YELLOW);
        std::cout << "SWITCH NETWORK STATUS:\n\n";
        
        // Visual switch topology
        for (int i = 0; i < switch_count; ++i) {
            // Switch box with activity indicator
            std::cout << "  ";
            ...
            std::cout << "Packets=" << std::setw(4) << switch_packets[i]->load() 
                      << ", MACs=" << std::setw(3) << mac_tables[i]->load();
            
            // Traffic indicator
            if (animation_enabled && switch_packets[i]->load() > 0) {
    ...
    void show_topology() {
        ...
        // Activity indicators on connections
    ...
    void show_final_stats(const PerformanceStats& stats, 
        ...
        for (int i = 0; i < switch_count; ++i) {
            std::cout << "  " << switch_names[i] << ": ";
            std::cout << "Packets=" << switch_packets[i]->load() << ", ";
            std::cout << "MACs=" << mac_tables[i]->load() << "\n";
        }
        std::cout << "\n";
        
        set_color(GREEN);
        std::cout << ">>> Mini SONiC REAL components working perfectly! <<<\n";
        set_color(WHITE);
    }
    ...
    void mark_switch_active(int switch_id) {
        if (switch_id >= 0 && switch_id < switch_count) {
            switch_activity[switch_id] = 3; // Show activity for 3 frames
        }
    }
...
// Real switch using actual Mini SONiC components
...
private:
    ...
    AnimatedVisualizer* visualizer;
    
public:
    AnimatedSwitch(int id, const std::string& switch_name, AnimatedVisualizer* viz) 
        ...
        visualizer->log_event(name + " initialized with real Mini SONiC components");
    }
...
// Main animated demo controller
class AnimatedNetworkDemo {
private:
    AnimatedDemoConfig config;
    PerformanceStats stats;
    AnimatedVisualizer visualizer;
    
    std::vector<std::unique_ptr<AnimatedSwitch>> switches;
...
public:
    AnimatedNetworkDemo(const AnimatedDemoConfig& cfg) 
        ...
        for (int i = 0; i < config.num_switches; ++i) {
            ...
            auto switch_ptr = std::make_unique<AnimatedSwitch>(i, name, &visualizer);
            switches.push_back(std::move(switch_ptr));
        }
        
        visualizer.show_header();
        std::cout << "Starting animated demo with " << config.num_switches << " switches...\n";
        std::cout << "Target rate: " << config.packets_per_second << " packets/second\n";
        std::cout << "Duration: " << config.demo_duration_seconds << " seconds\n";
        std::cout << "Using REAL Mini SONiC components with live animation\n\n";
    }
    ...
    void start() {
        ...
        // Start demo thread
        demo_thread = std::make_unique<std::thread>(&AnimatedNetworkDemo::run_demo, this);
        
        visualizer.log_event("DEMO STARTED - Real Network Processing");
    }
    ...
    void stop() {
        ...
        visualizer.log_event("DEMO COMPLETED");
    }
...
private:
    void run_demo() {
        ...
        auto update_interval = std::chrono::milliseconds(200); // Update every 200ms for smooth animation
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--switches" && i + 1 < argc) {
            ...
            std::cout << "Mini SONiC Animated Real Demo\n";
    ...
    try {
        // Create and run animated demo
        AnimatedNetworkDemo demo(config);
